...
function carregaEndereco() {
	...
	$usuario = $DB->query ( '
			SELECT endereco1 FROM usuario WHERE id = :usuario', array (
...
function carregaPreco() {
	...
	$pedido = $DB->query ( '
			SELECT preco FROM pedidos WHERE id = :idpedido', array (
...
function carregaNome() {
	...
	$usuario = $DB->query ( '
			SELECT nome FROM usuario WHERE id = :idusuario', array (
...
function carregaSobrenome() {
	...
	$usuario = $DB->query ( '
			SELECT sobrenome FROM usuario WHERE id = :idusuario', array (
			':idusuario' => $_SESSION ['idusuario'] 
	) )->fetch ();
