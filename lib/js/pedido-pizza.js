	...
	$('li', '#listaIngredientes').click(function() {
		...
		if (pizza.getTamanho() == null) {
			...
			pizza.adicionarIng(ingred, function(ing) {
				$('#ingadd tbody').append('<tr><td>'+ingred+'<input type="hidden" name="ing[]" value="'+ingred+'" /></td><td><img src='+imagem+'><input type="hidden" name="imging[]" value="'+imagem+'" /></td><td><center><button id="removeing" type="button" class="btn btn-danger btn-md" aria-label="Center Align"><span class="glyphicon glyphicon-minus" aria-hidden="true"></span></td></center></td></tr>');
			});
		}
